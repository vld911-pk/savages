...
function Exersice() {
  ...
  let [num1, num2, result] = useCallback(generateRandomNumbers(range), [
    score,
    passedTasks,
    range,
  ]);
  ...
  const compareAnswer = () => {
    if (passedTasks >= taskCount) {
      Promise.all([setPassedTasks(0), setScore(0)]);
      return;
    }
  ...
  return (
    <>
      ...
      <ExersiceWrapper>
        ...
        >
          <Paragraph params={"0px 15px 0px 15px"}>
            {passedTasks}/{parseInt(taskCount)} tasks
          </Paragraph>
          <Timer
